...
enum AllocType {
   ...
   * administrator to pre-reserve space for huge memory pages as documented in
   * the reported external references  */
  FORCEHUGEPAGE
};
...
public:
  ...
  explicit Vector(const T *data, size_t length) : Vector(length) {
  ...
  /** Trim the the memory allocated so that it holds at most the given number of
   * elements.
...
#else
        mem = mremap(data, page_aligned(_capacity * sizeof(T)), space,
                     MREMAP_MAYMOVE); //, -1, 0);
#endif
      }
