...
pub struct CompressedWordSet<const PREFIX_BITS: usize, const SUFFIX_BITS: usize>
where
    [(); SUFFIX_BITS.div_ceil(8)]:,
{
    prefixes: RankBitContainer,
    tiered: UniquePtr<TieredVec28>,
    suffix_containers: Vec<
        SemiCompressed<
            { SUFFIX_BITS.div_ceil(8) },
            64,
...
impl<const PREFIX_BITS: usize, const SUFFIX_BITS: usize> CompressedWordSet<PREFIX_BITS, SUFFIX_BITS>
where
    [(); SUFFIX_BITS.div_ceil(8)]:,
{
    ...
    pub fn count(&self) -> usize {
        self.suffix_containers
            .iter()
            .map(|container| container.len(&self.compressor))
            .sum()
    }
    ...
    #[inline]
    pub fn contains<T: PrimInt + Unsigned + AsPrimitive<usize>>(&mut self, word: T) -> bool {
    ...
    pub fn insert<T: PrimInt + Unsigned + AsPrimitive<usize>>(&mut self, word: T) -> bool {
        ...
        if absent {
            match self.empty_containers.pop() {
                Some(id) => {
                    self.suffix_containers[id].insert(&mut self.compressor, suffix);
                    self.tiered.insert(rank, id as u32);
                }
                None => {
                    let id = self.suffix_containers.len();
                    self.suffix_containers.push(SemiCompressed::<
                        { SUFFIX_BITS.div_ceil(8) },
                        64,
                        SemiSortedVec<CompactInt<{ SUFFIX_BITS.div_ceil(8) }>, 32>,
                        SnapCompressor,
                    >::new_with_one(
                        &mut self.compressor, suffix
                    ));
                    self.tiered.insert(rank, id as u32);
                }
            };
            ...
            let id = self.tiered.get(rank) as usize;
            absent = self.suffix_containers[id].insert(&mut self.compressor, suffix);
        }
        absent
    }
    ...
    pub fn insert_batch<T: PrimInt + Unsigned + AsPrimitive<usize>>(&mut self, words: &[T]) {
        ...
        for group in prefixes_suffixes.group_by(|(p1, _), (p2, _)| p1 == p2) {
            ...
            if group.len() == 1 {
                self.suffix_containers[id].insert(&mut self.compressor, group[0].1);
            } else {
                self.suffix_containers[id].insert_iter(
                    &mut self.compressor,
                    group.iter().map(|&(_, suffix)| suffix),
                );
            }
        }
    }
...
impl<const PREFIX_BITS: usize, const SUFFIX_BITS: usize> Default
    for CompressedWordSet<PREFIX_BITS, SUFFIX_BITS>
where
    [(); SUFFIX_BITS.div_ceil(8)]:,
{
    fn default() -> Self {
...
mod tests {
    ...
    fn test_insert_contains_remove() {
        ...
        let mut set = CompressedWordSet::<PREFIX_BITS, SUFFIX_BITS>::new();
